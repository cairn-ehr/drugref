...
project's forty-odd modules, so rule 4 was enforced on the files that happened to
...
named below with their measured size, and the second test asserts that each one
is STILL over: a module that gets split and then quietly stays on the list would
turn the ledger into a permanent exemption, which is the thing an allow-list
always decays into. Removing a name is part of splitting the file.
"""
import pathlib
...
#: the size measured that day. Nothing here was written or grown by the round that
...
#: split (`spl_evidence.py`, 518 -> 428, `Registry`/`load_registry` moved to
