...
same as test_medrt_mesh_ci_parser.py does: they build a MED-RT file, and there is one
way to do that here, not two.
"""
import pathlib
...
from tests.test_medrt_parser import assoc, concept, write_medrt
...
# The single MoA concept every controlled-input test below associates against. These
# tests exercise ASSOCIATION parsing, not concept parsing, so one fixed concept
# (mirroring test_medrt_parser's own "C-MOA" / "N-MOA-9" pair) is enough for all of them.
MOA_CONCEPT = concept("C-MOA", "N-MOA-9", "Real Mechanism [MoA]", cty="MoA")
...
def test_each_predicate_is_parsed_with_its_raw_mesh_code(tmp_path, predicate):
    path = write_medrt(tmp_path, MOA_CONCEPT,
                       assoc(predicate, "RxNorm", "161", "MeSH", "M0001"))
...
def test_a_class_subject_is_refused_and_counted(tmp_path):
    ...
    path = write_medrt(tmp_path, MOA_CONCEPT,
                       assoc("may_treat", "MED-RT", "C-MOA", "MeSH", "M0001"))
    result = medrt.parse(path)
