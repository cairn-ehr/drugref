...
`analyze.analyze_tables` refuses unless the server actually did the work — on the collected WARNING **and** on
`pg_class.reltuples` still reading `-1`, two checks that are not one check twice: only the first sees a
re-ingest, only the second sees a connection whose notices go nowhere. Verified at full scale on both sides of
...
WRITER of the SPL projection — moved to `registry_read.py`, `spl_evidence.py` **518 → 428**. The cap test the
issue asked for became a **sweep**: `500` had lived in two test files that between them guarded three modules of
forty-odd, which is how 518 happened with a green suite; `tests/test_module_size_cap.py` now owns the number
once and checks every module under `src/drugref`, with the seven already over it as a **checked ledger**
(a second test fails if a listed module comes back under the cap) — [#177](https://github.com/cairn-ehr/drugref/issues/177).
