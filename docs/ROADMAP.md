...
### The DrugCentral re-measurement ✅ DONE (2026-08-23) — no migration, no ingest, suite 1794 → 1828
...
`tools/` as three modules with **34 tests**, and it re-runs in ~40 s from a cached extract.
...
own free text to a `struct_id` for 922 of 924 NDF-RT endpoint names, and `structures` carries an InChIKey and a
...
6,866** — with no hand-maintained synonym list, which is what #101 had budgeted for. **Six published figures were
wrong** (the "8 MED-RT class names" is 4 MeSH ones; 102 → 106; 7,000 → 6,991; `MAOIs or RIMAs` is not in the
data; the QT strings; the QT rows all sit in the licence-excluded subset), all corrected in place.
