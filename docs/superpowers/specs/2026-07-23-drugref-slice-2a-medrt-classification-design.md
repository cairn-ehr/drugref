...
3. **The parser is not the only distribution channel — the repository is one too.** Blocking an edge keeps
   unlicensed content out of the *database*, but a **test fixture** naming that edge's far endpoint
   redistributes the term the moment it is committed, whatever the parser later does with it. The fixture
   extractor therefore redacts the term and code of every endpoint outside MED-RT/RxNorm before writing
   (`tests/fixtures/make_medrt_subset.py`), keeping the namespace — which is what the parser rejects on — so
   the fixture loses none of its discriminating power. A test asserts the committed fixture stays redacted,
   because the fixture is regenerated by hand and would otherwise silently reacquire the terms.
...
- **`substance_class`** — `class_uuid UUID PRIMARY KEY`, `medrt_nui TEXT NOT NULL UNIQUE` (natural key),
  ...
  Thin, mirroring `substance_moiety`. **`medrt_nui` and `medrt_code` are distinct fields on purpose**: the
  NUI is *identity* (`class_uuid` derives from it) while the code is what association endpoints are
  *referenced by*. They carry the same string throughout the 2026.07.06 release, so treating them as one
  works today — and would fail silently the day upstream let them diverge, matching no edge at all and
  rebuilding an empty DAG with no error to show for it. The parser resolves endpoints code → NUI.
...
- **Only MED-RT-namespace concepts are *defined* in the file**, all with `status = 'A'`. RxNorm, MeSH and
  ...
  exactly what the parser's endpoint filter blocks. The parser nonetheless **requires** `status = 'A'`
  rather than assuming it: `substance_class` never deletes, so a release that began publishing retired
  concepts would seed dead classes permanently. Refusals are counted, not dropped silently.
...
- **Membership join.** Each `has_*` assertion links a *drug ingredient concept* (an RxNorm-namespace
  ...
  `identity_claim(scheme='RXNORM_IN', value=rxcui, superseded_by IS NULL)` → `moiety_uuid`, taking **every**
  match rather than the first: `identity_claim` is unique on `(moiety_uuid, scheme, value)`, so two moieties
  may claim one RxCUI, and an unordered single-row read would both drop a real membership and make the
  ingest non-reproducible. (`chebi.py` settled the identical question the identical way for InChIKey.)
  **Unmatched** ingredients (moiety not in our registry, e.g. gated out by has-INN) are **skipped and
  counted** — the run summary reports the skip count as a worklist number, never a silent drop (mirrors the
  slice-1 gate's audit posture: an active-looking substance with no match is a worklist item, not an
  invisible exclude). The index is read **once per run**, not once per assertion: ~27,500 memberships over
  ~6,000 ingredients means four lookups in five would otherwise re-ask an answered question.
- **Orchestrator.** Open an `ingest_run` (`source='MED-RT'`, `upstream_release`, checksum); upsert classes;
  **rebuild** (`DELETE` this source's prior rows, then insert) `class_parent` + `class_membership` for the
  run; return a summary. The summary separates **classes-in-release** from **classes-added** because the
  tables behave differently — classes accumulate, edges are rebuilt — so one number would be ambiguous on a
  re-ingest; alongside them the isa-edge and membership row counts, and three worklist numbers
  (RxCUI-unmatched, inactive concepts, unidentified concepts).
