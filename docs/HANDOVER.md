...
Migrations through **`db/048`** are frozen and **this round adds none**. Suite **1,828** (1,794 at branch start).
...
measurement lives in `tools/` as three modules with **34 tests**, and it re-runs in ~40 s from a cached extract.
...
`struct_id` for 922 of 924 NDF-RT endpoint names, and `structures` carries an InChIKey and a CAS number drugref
...
**Six published figures were wrong and are corrected in place** in PROJECT-NOTES and ROADMAP: "8 MED-RT class
...
`Moderate Risk QT Prolonging Agents`; and **all three QT rows are `ddi_ref_id = 3`**, so issue #93's content sits
in the half rule 6 forbids. Everything on the moiety side — 7,621 / 970 / 860 / 6,973 / 6,941 / 604 / 6,337 —
