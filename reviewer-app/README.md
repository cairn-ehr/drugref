...
Accounts, sessions, the paginated clinical queue and append-only working records are live through
...
references; these do not change clinical state or create a ruling. Decision and signing
controls remain disabled, and authentication is not a clinical signature.
...
`src/lib/presentation.ts`; `src/lib/records.ts` owns the native/browser-preview
working-record adapters. Components retain only lifecycle, event, and view state.
